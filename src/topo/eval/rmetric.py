...
class RiemannMetric:
    ...
    def get_detG(self, use_log=True):
        ...
        if self.Hsvals is None:
            raise RuntimeError("Riemann metric singular values are not available.")
        Hsvals = self.Hsvals
        if use_log:
            self.detG = -np.sum(np.log(Hsvals + 1e-8), axis=1)
            return self.detG
        else:
            self.detG = np.prod(1.0 / (Hsvals + 1e-8), axis=1)
            return self.detG
...
def _prepare_colors(c, n, cmap="viridis", vmin=None, vmax=None, default_alpha=None):
    ...
    try:
        ...
        rgba = np.array([mcolors_mod.to_rgba(ci) for ci in c], float)
        if rgba.shape[0] == n:
            if default_alpha is not None:
                rgba[:, 3] = default_alpha
            return rgba
    ...
    base = cm_mod.get_cmap("tab20" if len(labels) > 10 else "tab10")
    palette = np.array(
...
def plot_riemann_metric_localized(
    ...
    if show_points:
        if rgba_all is not None:
            kw: dict[str, Any] = {"s": point_size, "zorder": zorder - 2}
            if scatter_kw:
                kw.update(scatter_kw)
            cast(Any, ax).scatter(Y[:, 0], Y[:, 1], c=rgba_all, **kw)
        else:
            kw: dict[str, Any] = {
                "s": point_size,
                "c": "0.3",
            ...
            if scatter_kw:
                kw.update(scatter_kw)
            cast(Any, ax).scatter(Y[:, 0], Y[:, 1], **kw)
...
def plot_riemann_metric_global(
    ...
    if _HAVE_SK and NearestNeighbors is not None:
    ...
    if show_points:
        ...
        if scatter_kw:
            kw.update(scatter_kw)
        cast(Any, ax).scatter(Y_plot[:, 0], Y_plot[:, 1], **kw)
    ...
    mappable.set_array([])
    cb = plt_mod.colorbar(mappable, ax=ax, fraction=0.046, pad=0.04)
...
def calculate_deformation(
    ...
    G_arr = np.asarray(cast(Any, G_emb), dtype=float)

    if use_dual:
        # equivalently: val = -logdet(H)
        lam = np.linalg.eigvalsh(G_arr)  # treat input as H if provided
        lam = np.clip(lam, 1e-12, None)
        logdet = -np.sum(np.log(lam), axis=1)
    else:
        lam = np.linalg.eigvalsh(G_arr)
        lam = np.clip(lam, 1e-12, None)
        logdet = np.sum(np.log(lam), axis=1)
...
def plot_metric_contraction_expansion(
    ...
    from topo.eval.rmetric import _center as _ct
    from topo.eval.rmetric import _symmetrize as _sz
    ...
    if show_colorbar:
        cb = plt_mod.colorbar(sc, ax=ax, fraction=0.046, pad=0.04)
        cb.set_label(
